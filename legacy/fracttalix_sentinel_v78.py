# fracttalix_sentinel v7.8 py

# Fracttalix Sentinel v7.8 — Deployment-Ready: Auto-Tune, Multi-Stream, Visualization
...
# v7.8: Three deployment gaps closed:
#   - auto_tune() classmethod: data-driven hyperparameter search targeting a
#     user-specified false-positive rate; returns a warmed-up detector + report
#   - MultiStreamSentinel: fan-out manager for N independent streams; thread-parallel
#     batch updates via concurrent.futures; full state save/load across all streams
#   - plot_history(): optional matplotlib dashboard (3-panel: value+thresholds+alerts,
#     turbulence+rhythm, fractal+resilience); guarded import, graceful if absent
#   - history ring-buffer (history_maxlen param) accumulates compact per-step records
#     for visualization and post-hoc inspection
#   - All v7.7 features, parameters, and JSON state fields preserved (backward-compatible)
...
class Detector_7_8:
    """
    Enhanced lightweight, regime-aware anomaly detector — v7.8.

    Extends v7.7 with three deployment capabilities and the full FRM axiom suite:
    ...
    def auto_tune(
        ...
        verbose: bool = False,
        **fixed_kwargs,
    ) -> Tuple["Detector_7_8", Dict[str, Any]]:
        """
        ...
        **fixed_kwargs: passed directly to Detector_7_8 (e.g. warm_up_period)

        Returns
        -------
        (detector, report)
            detector : Detector_7_8 warmed up on all of `data`, best params set
...
class MultiStreamSentinel:
    """
    Fan-out manager for N independent Detector_7_8 instances.
    ...
    def get_detector(self, stream_id: str) -> Optional[Detector_7_8]:
        """Return the Detector_7_8 for stream_id, or None if unknown."""
        ...
        with self._global_lock:
            for sid, state in snapshot.items():
                if sid not in self._detectors:
                    self._detectors[sid] = Detector_7_8(**self._defaults)
...
if __name__ == "__main__":
    ...
    print("Fracttalix Sentinel v7.8 — Smoke test")
    print("=" * 55)
    ...
    det_tuned, report = Detector_7_8.auto_tune(
        tune_data, target_fpr=0.05, verbose=False,
        alpha_grid=[0.05, 0.10, 0.15],
        early_mult_grid=[2.5, 3.0, 3.5],
    )
    assert isinstance(det_tuned, Detector_7_8), "auto_tune must return Detector_7_8 instance"
    ...
    print("\n" + "=" * 55)
    print("All 13 tests passed. Sentinel v7.8 operational.")
    print("\nv7.8 additions verified:")
