# fracttalix_sentinel v7.10 py

# Fracttalix Sentinel v7.10 — Best-in-Class: PE, RSI, DES, Variance CUSUM, Seasonal Baseline, VUS-PR
...
class Detector_7_10:
    ...
    def _build_alert_reasons(
        ...
        if self._pe < 0.3 and self.pe_enabled:
    ...
    def _initialize_from_warmup(self) -> None:
        ...
        # v7.10: initialize baseline variance for VCUSUM; init seasonal model
        if self.baseline_std is not None:
            self._baseline_var = self.baseline_std ** 2
        if self.seasonal_enabled and len(self._phase_ewmas) == 0:
            self._init_seasonal()
        # v7.10: set baseline variance for Variance CUSUM
        if self.baseline_std is not None:
    ...
    def plot_history(
        self,
        last_n: int = 300,
        title: str = "Fracttalix Sentinel v7.10 — Dashboard",
        show: bool = True,
...
# ─────────────────────────────────────────────────────────────────────────────
# SMOKE TEST — 28 structured assertions covering all v7.10 features
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 55)
    print("All 28 tests passed. Sentinel v7.10 operational.")
    print("\nv7.10 additions verified (Meta-Kaizen v7.10):")
    ...
    print("  [v7.10] Naive 3σ baseline comparison  — reference in run_suite()")
    print("  [v7.10] 'drift' + 'variance' archetypes — tests PH and VCUSUM")
    print("  [v7.10] auto_tune() F1 mode           — labeled_data parameter")
    print("  [v7.10] plot_history() 4-panel        — adds PE + anomaly_score panel")
